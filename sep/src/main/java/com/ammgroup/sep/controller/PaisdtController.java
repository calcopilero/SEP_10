    ...
    void bexecOnAction(ActionEvent event) {
		...
		if (checkControls()) {
	        ...
	        case ADD :
	        	...
	        	cont = paisRepo.countExistingPaises(mutils.obtainText(tdesc));
	        	
	        	if (cont == 0) {
	        		pa = new Pais(mutils.obtainText(tdesc));
	        		paisRepo.save(pa);
	        	} else {
	        ...
	        case EDIT:
	        	...
	        	cont = paisRepo.countExistingPaises(mutils.obtainText(tdesc), paiscrud.getDao().getId());
	        	
	        	//Check if there are no coincidences
	        	if (cont == 0) {
	        		pa = paiscrud.getDao();
	        		pa.setDescripcion(mutils.obtainText(tdesc));
	        		paisRepo.save(pa);
	        	} else {
	        		lbmsg1.setText("Existen " + String.valueOf(cont) + " paises con esa descripción.");
	        		cform = false;
